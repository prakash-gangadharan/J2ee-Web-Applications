...
app.controller("CountryController", function($scope, $http) {
	...
	function _refreshCountryData() {
		$http({
			method : 'GET',
			url : 'http://localhost:8081/RestWithAngularJS/rest/countries'
	...
	$scope._testConsumesData = function() {
		...
		$http({
			method : 'POST',
			url : 'rest/WebServiceControllerTwo/Consumes',
			data : angular.toJson($scope.countryForm),
			headers : {
				'Content-Type' : 'application/json'
			} 
	...
	$scope._testPathParamData = function() {
		...
		$http({
			method : 'GET',
			url : 'rest/WebServiceControllerTwo/' + $scope.countryForm.id
		}).then(function successCallback(response) {
			console.log(response.data);
		}, function errorCallback(response) {
			console.log(response.statusText);
	...
	$scope._testQueryParamData = function() {
		...
		$http({
			method : 'GET',
			url : 'rest/WebServiceControllerTwo',
			params    : {
	...
	$scope._testHeaderParamData = function() {
		...
		$http({
			method : 'GET',
			url : 'rest/WebServiceControllerTwo/query',
			params    : {
				nameKey: $scope.formData.nameKey,
				countryKey: $scope.formData.countryKey
				},  // pass in data as strings
